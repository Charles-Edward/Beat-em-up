...
public class PlayerBehaviour : MonoBehaviour
{
    ...
    // l'utilisation de Mathf.abs permet d'avoir une valeur toujours positive, aller vers la gauche = valeur n�gative en X et on passe cette valeur n�gative en valeur positive
    // pour voir entrer dans les conditions de l'animator (ex : Transition si MoveSpeed > .1, si on va vers la gauche on aura une valeur n�gative et on entrera
    // jamais dans cette condition
    // l'utilisation de Mathf.Max permet de r�cup�rer la valeur la plus grande entre X et Y, donc si on va vers le haut on aura 0,1,0 on n'utilise donc pas X alors on va se servir
    ...
    void OnStateUpdate()
    {
        ...
        switch (_currentState)
        {
            ...
            case PlayerStateMode.SPRINT:
                ...
                if (Input.GetButtonUp("Fire3"))  // si on arr�te d'appuyer sur sprint on repasse en walk
                {
                ...
                else if (Input.GetButtonDown("Jump")) // permet de sauter dans l'�tat sprint
                {
            ...
            case PlayerStateMode.JUMP:
                ...
                {
                    _jumpTimer = 0f;
                    if (_magnitude > 0) // si la vraie vitesse (magnitude) est sup�rieure � 0 on marche
                    {
                        TransitionToState(PlayerStateMode.WALK);
                    }
            ...
            case PlayerStateMode.BASICPUNCH:
                ...
                if (Input.GetButtonUp("Fire1")) // si on arr�te d'attaquer on passe en idle
                {
                    TransitionToState(PlayerStateMode.IDLE);
